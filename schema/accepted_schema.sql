...
--
-- Overall schema hash: a22731092d3f9f7c414a9695bbc8c6a9

-- Accepted on 2023-04-04T18:05:41.000Z by 20230404T180541.make_posts_searchable.ts
...
-- Schema for "Posts", hash: 8fd815257ce6457e682b6f26bfb26287
CREATE TABLE "Posts" (
    _id varchar(27) PRIMARY KEY,
    "postedAt" timestamptz,
    ...
    "searchVector" tsvector GENERATED ALWAYS AS (SETWEIGHT(TO_TSVECTOR('english',
    LEFT (COALESCE("title", ''), 1024 * 1024)), 'A') || SETWEIGHT(TO_TSVECTOR('english',
    LEFT (REGEXP_REPLACE(COALESCE("contents" ->> 'html', ''), E'<[^>]+>', '', 'gi'), 1024 * 1024)), 'B')) STORED,
    "voteCount" double precision DEFAULT 0,
    "baseScore" double precision DEFAULT 0,
    "extendedScore" jsonb,
    "score" double precision DEFAULT 0,
