...
import { DefaultPreview, ViewpointsPreview, EstimakerPreview, ArbitalPreview, OWIDPreview, MetaforecastPreview, NeuronpediaPreview, FatebookPreview, ManifoldPreview, MetaculusPreview, MozillaHubPreview } from "@/components/linkPreview/PostLinkPreview";
import FootnotePreview from "@/components/linkPreview/FootnotePreview";
...
const HoverPreviewLink = ({ href, contentSourceDescription, id, rel, noPrefetch, children }: {
  ...
  if (href.startsWith("#")) {
    if (locationHashIsFootnote(href)){
      return <FootnotePreview href={href} id={id} rel={rel}>
        {children}
      </FootnotePreview>
  ...
  try {
    ...
    if (!linkIsExcludedFromPreview(onsiteUrl) && (hostType==="onsite" || hostType==="mirrorOfUs" || isServer)) {
      ...
      if (linkTargetAbsolute.host === "hubs.mozilla.com") {
        return <MozillaHubPreview href={href} id={id}>
          {children}
        </MozillaHubPreview>
      }
...
export default HoverPreviewLinkComponent;
