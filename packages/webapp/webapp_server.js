...
function runWebAppServer() {
  ...
  app.use(async function (req, res, next) {
    console.log(`${new Date().getTime()}: Entering the final Meteor request handler`);
    if (! appUrl(req.url)) {
      return next();
      ...
      return getBoilerplateAsync(request, arch, flush).then(({
        ...
        console.log(`${new Date().getTime()}: Calling res.writeHead`);
        res.writeHead(statusCode, headers);

        console.log(`${new Date().getTime()}: Piping into response`);
        stream.pipe(res, {
