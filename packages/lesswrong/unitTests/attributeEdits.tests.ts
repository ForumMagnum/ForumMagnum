...
describe('Diff attribution tracking', () => {
  ...
  it('attributeEdits', () => {
    chai.assert.deepEqual(
      attributeEdits("<div>Asdf</div>", "<div>Asdf ghjk</div>", "2", [...times(4, ()=>"1")]),
      [...times(4, ()=>"1"), ...times(5, ()=>"2")]
    );
    
    chai.assert.deepEqual(
      attributeEdits("<div> Asdf</div>", "<div> Asdf ghjk</div>", "2", [...times(5, ()=>"1")]),
      [...times(5, ()=>"1"), ...times(5, ()=>"2")]
    );
    
    chai.assert.deepEqual(
      attributeEdits("<div> Asdf ghjk</div>", "<div> Asdf</div>", "2", [...times(10, ()=>"1")]),
      [...times(5, ()=>"1")]
    );
    
    chai.assert.deepEqual(
      attributeEdits("<div> <span>x</span> Asdf</div>", "<div>  Asdf ghjk</div>", "2", [...times(7, ()=>"1")]),
      [...times(6, ()=>"1"), ...times(5, ()=>"2")]
    );
    
    chai.assert.deepEqual(
      attributeEdits("<div>x y</div>", "<div>x  z</div>", "2", [...times(3, ()=>"1")]),
      [...times(3, ()=>"1"), "2"]
    );
    ...
    chai.assert.deepEqual(
      attributeEdits(
        ...
        "3",
        [...times(6, ()=>"1"), ...times(6, ()=>"2")]),
      [...times(6, ()=>"1"), ...times(6, ()=>"2"),  ...times(17, ()=>"3")]
    );
  });
  it("doesn't crash on test case reduced derived from rationality tag 1.1.0", () => {
    ...
    attributeEdits(oldHtml, newHtml, "user2", [...times(oldHtml.length, ()=>"user1")]);
  });
