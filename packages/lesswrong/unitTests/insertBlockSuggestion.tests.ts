...
const footnoteTestCases: FootnoteTestCase[] = [
  {
    name: "named footnotes in edit mode",
    markdown: "Body with a footnote.[^note]\n\n[^note]: Definition **body** with a [link](https://example.com).",
    definitionText: "Definition body with a link.",
    mode: "edit",
  },
  {
    name: "inline footnotes in edit mode",
    markdown: "Body with an inline footnote.^[Inline definition text.]",
    definitionText: "Inline definition text.",
    mode: "edit",
  },
  {
    name: "named footnotes in suggestion mode",
    markdown: "Body with a footnote.[^note]\n\n[^note]: Definition **body** with a [link](https://example.com).",
    definitionText: "Definition body with a link.",
    mode: "suggest",
  },
  {
    name: "inline footnotes in suggestion mode",
    markdown: "Body with an inline footnote.^[Inline definition text.]",
    definitionText: "Inline definition text.",
    mode: "suggest",
  },
...
describe("insertBlock with footnotes", () => {
  it.each(footnoteTestCases)("preserves $name", async ({ markdown, definitionText, mode }) => {
    const editor = await setupEditorWithContent("Existing paragraph.");
    await insertBlock(editor, markdown, "end", mode);
    ...
    editor.getEditorState().read(() => {
      ...
      expect(itemChildren[1].getTextContent().replace(/\s+/g, " ").trim()).toBe(definitionText);
    ...
    expect(getMarkdownContent(editor)).toContain(`: ${definitionText}`);
  });
});
