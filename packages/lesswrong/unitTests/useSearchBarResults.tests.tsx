...
import { useSearchBarResults } from "../components/search/useSearchBarResults";
...
it("appends pages and prevents duplicate requests while a page is pending", async () => {
  ...
  const {result} = renderHook(() => useSearchBarResults("posts,users", "test", true));
...
it("ignores stale results when query or content kinds change", async () => {
  ...
  const {result, rerender} = renderHook(({index, query}) => useSearchBarResults(index, query, true), {
...
it("retries a failed page without skipping it and does not fetch while closed", async () => {
  ...
  const {result, rerender} = renderHook(({open}) => useSearchBarResults("posts", "test", open), {
