...
describe("SelectQuery", () => {
  runTestCases([
    ...
    },
    {
      ...
      expectedSql: 'SELECT "TestCollection".* FROM "TestCollection" WHERE "a" ::DOUBLE PRECISION IN ( $1 , $2 , $3 )',
      expectedArgs: [1, 2, 3],
    },
    {
      name: "can build select query with not-in comparison",
      getQuery: () => new SelectQuery(testTable, {a: {$nin: [1, 2, 3]}}),
      expectedSql: 'SELECT "TestCollection".* FROM "TestCollection" WHERE NOT ( "a" ::DOUBLE PRECISION IN ( $1 , $2 , $3 ) )',
      expectedArgs: [1, 2, 3],
    },
    {
