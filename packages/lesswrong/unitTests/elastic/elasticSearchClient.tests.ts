...
import { executeSearch } from "../../server/search/elastic/ElasticClient";
...
it("sends advanced searches directly to the established filtered query", async () => {
  ...
  await executeSearch(client, {indexes: ["posts"], search: 'user:ey "corrigibility"'});
  expect(search).toHaveBeenCalledTimes(1);
  expect(JSON.stringify(search.mock.calls[0][0])).toContain("authorSlug.sort");
});

it("runs one person lookup and one ranked search before applying pagination", async () => {
  const client = new Client({node: "http://localhost:9200"});
  ...
  const result = await executeSearch(client, {indexes: ["users", "posts", "sequences"], search: "Eliezer", offset: 10, limit: 5});
...
it("fails loudly when any shard failed instead of returning a partial ranking", async () => {
  ...
  await expect(executeSearch(client, {indexes: ["posts"], search: "alignment"})).rejects.toThrow(/shard/i);
});
...
it("rejects a timed-out person lookup before ranking with incomplete author candidates", async () => {
  ...
  await expect(executeSearch(client, {indexes: ["posts"], search: "Paul"})).rejects.toThrow(/timed out/i);
  expect(search).toHaveBeenCalledTimes(1);
...
it("rejects a timed-out final search even when all shards report success", async () => {
  ...
  await expect(executeSearch(client, {indexes: ["posts"], search: '"lab automation"'})).rejects.toThrow(/timed out/i);
});
