    ...
    resolveAs: {
      ...
      resolver: async (document: DbRevision, args: void, context: ResolverContext): Promise<DbRevision["originalContents"]|null> => {
        ...
        if (document.collectionName === "Posts") {
          const post = await context.loaders["Posts"].load(document.documentId!) // TODO: technically nullable field, even though in practice it's not.  Need to update db schema.
  ...
  tag: resolverOnlyField({
    ...
    resolver: async (revision: DbRevision, args: void, context: ResolverContext) => {
      ...
      if (revision.collectionName !== "Tags")
        return null;
      const tag = await context.loaders.Tags.load(revision.documentId!); // TODO: technically nullable field, even though in practice it's not.  Need to update db schema.
  ...
  post: resolverOnlyField({
    ...
    resolver: async (revision: DbRevision, args: void, context: ResolverContext) => {
      ...
      if (revision.collectionName !== "Posts")
        return null;
      const post = await context.loaders.Posts.load(revision.documentId!); // TODO: technically nullable field, even though in practice it's not.  Need to update db schema.
