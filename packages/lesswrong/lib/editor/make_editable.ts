...
const buildEditableResolver = <N extends CollectionNameString>(
  ...
  if (normalized) {
    return {
      ...
      resolver: async function normalizedEditableResolver(
      ...
      sqlResolver: function normalizedEditableSqlResolver({field, resolverArg, join}: SqlResolverArgs<N>) {
        return join({
          table: "Revisions",
          type: "left",
          /**
  ...
  return {
    type: "Revision",
    arguments: "version: String",
    resolver: async function denormalizedEditableResolver(
      doc: ObjectsByCollectionName[N],
...
function getDefaultLocalStorageIdGenerator<N extends CollectionNameString>(collectionName: N) {
...
export function editableFields<N extends CollectionNameString>(collectionName: N, options: MakeEditableOptions<N> = {}): Record<string, CollectionFieldSpecification<N>> {  
  ...
  async function revisionsResolver(
  ...
  function versionResolver(doc: ObjectsByCollectionName[N]): string {
