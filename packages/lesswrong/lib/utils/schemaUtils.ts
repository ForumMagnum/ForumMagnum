...
const generateIdResolverSingle = ({collectionName, fieldName}) => {
  return async (doc, args, context) => {
    ...
    return await accessFilterSingle(currentUser, collection, resolvedDoc);
  }
}
...
const generateIdResolverMulti = ({collectionName, fieldName, getKey = (a=>a)}) => {
  return async (doc, args, context) => {
    ...
    return await accessFilterMultiple(currentUser, collection, resolvedDocs);
  }
}
...
export const accessFilterSingle = async <T extends DbObject>(currentUser: DbUser|null, collection: CollectionBase<T>, document: T|null): Promise<T|null> => {
  const { checkAccess } = collection
  if (!document) return null;
  if (checkAccess && !(await checkAccess(currentUser, document))) return null
...
// view.
export const accessFilterMultiple = async <T extends DbObject>(currentUser: DbUser|null, collection: CollectionBase<T>, unfilteredDocs: Array<T|null>): Promise<Array<T>> => {
  const { checkAccess } = collection
  ...
  const filteredDocs = checkAccess ? await asyncFilter(existingDocs, async (d: T) => await checkAccess(currentUser, d)) : existingDocs
