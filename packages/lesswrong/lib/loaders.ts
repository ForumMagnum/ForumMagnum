 ...
 */
export async function getWithLoader<T extends DbObject>(
  context: ResolverContext,
  collection: CollectionBase<T>,
  loaderName: string,
  baseQuery: MongoSelector<T> = {},
  groupByField: keyof T,
  id: string,
  findOptions?: MongoFindOptions<T>,
): Promise<Array<T>> {
  ...
  if (!context.extraLoaders[loaderName]) {
    context.extraLoaders[loaderName] = new DataLoader(async (docIDs: Array<string>) => {
      ...
      const queryResults = await collection.find(query, findOptions).fetch();
