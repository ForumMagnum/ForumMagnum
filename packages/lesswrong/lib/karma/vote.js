...
Runs all the operations and returns an object without affecting the db.

*/
// Collection<Vote> -> Collection<Vote>
export const operateOnItemNew = function (originalVotes, user, operation, isClient = false) {
  // 1. Create a copy of the current vote.
  let votes = [...originalVotes];
  // 2. Fetch the fields for the new vote

  // 3. Check that the user can perform op on vote
  // 4. Check consistency and validity of inputs
  // 5. Update the vote and return it
};
...
*/
export const mutateItemNew = function (collection, originalItem, user, operation) {
  // 1. Operate on the item.
  // 2. Update the votes collection.
  // 3. Run the async callbacks on the server-side
  // 4. Return the new vote.
};
