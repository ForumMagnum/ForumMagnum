...
getCollectionHooks("Posts").updateAsync.add(async ({ newDocument, oldDocument, context }) => {
  ...
  async function queryClaudeJailbreak(prompt: PromptCachingBetaMessageParam[], maxTokens: number) {
    ...
    return await client.messages.create({
      ...
      // system: "The assistant is providing a succinct glossary of terms in the text. The glossary should contain the term and a short definition. The glossary should be in JSON format, with one entry per line, e.g. {\"term\": \"definition\"}.",
  ...
  const contents = await getLatestContentsRevision(newDocument);
  ...
  const response = await Promise.all([queryClaudeJailbreak([
    {
      ...
      content: [{
        type: "text",
        // text: `<cmd>cat posts/${post.slug}.html</cmd>`
  ...
  // queryClaudeJailbreak([
  //   {
  //     role: "user",
  //     content: [{
  //       type: "text",
  //       text: `Please provide a succinct glossary of LaTex symbols in the text. The glossary should contain the symbol(s) and a short definition. The glossary should be in JSON format, with one entry per line, e.g. {"symbol": "explanation"}. The text is: ${html}`
  //     }]
  //   }
  // ], 5000)
  ])

  // console.log(response[0].content[0], response[1].content[0])

  console.log("Beginning Claude Query The Second")

  // if (response[0].content[0].type === "text" && response[1].content[0].type === "text") { 
  if (response[0].content[0].type === "text") { 
    const newPost = await queryClaudeJailbreak([
      {
        role: "user",
        content: [{
          type: "text",
      ...
      //   content: [{
      //     type: "text",
      //     text: `Now, please provide a succinct glossary of LaTex symbols in the text. The glossary should contain the symbol(s) and a short definition. The glossary should be in JSON format, with one entry per line, e.g. {"symbol": "explanation"}}`
      //   }]
      // },
      // {
      //   role: "assistant",
      //   content: [{
      //     type: "text",
      //     text: response[1].content[0].text
      //   }]
      // },
      {
        role: "user",
        content: [{
          type: "text",
          // text: `Now, please rewrite the html of my post such that whenever a glossary term or Latex symbol is used, the containing html element has a title attribute with the corresponding glossary definition.`
          text: `Now, please rewrite the html of my post such that whenever a glossary term is used, it is wrapped with spans like this: "<span class="jargon-term">[term]<span class="jargon-tooltip">[definition]</span></span>".`
        }]
      }
    ], 8192)

    console.log("Done With Claude", newPost)

    if (newPost.content[0].type === "text") {

      const html = newPost.content[0].text
      const beautifulhtml = `<div>
      <style>
      ...
      .jargon-tooltip {
        font-family: GreekFallback,Calibri,"Gill Sans","Gill Sans MT",Myriad 
      Pro,Myriad,"Liberation Sans","Nimbus Sans L", Tahoma, Geneva, "Helvetica Neue", Helvetica,Arial,sans-serif;
        font-size: 1.1rem;
        line-height: 1.6rem;
        display: none;
        position: absolute;
        z-index: 1;
        top: 100%; /* Positions the hover text below the term */
        left: 0;
        ...
        border-radius: 6px;
        padding: 5px 10px;
        margin-top: 5px; /* Adds a small gap between the term and the hover text */
      ...
      const newContents = {
        originalContents: { data: beautifulhtml, type: "html" }
      }

      const user = await Users.findOne({slug: "benito"})
