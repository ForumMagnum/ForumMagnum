...
import { MODERATOR_ACTION_TYPES, RECEIVED_SENIOR_DOWNVOTES_ALERT, reviewTriggerModeratorActions } from "@/lib/collections/moderatorActions/constants";
...
export async function triggerReviewAfterModeration({ newDocument, currentUser, context }: AfterCreateCallbackProperties<'ModeratorActions'>) {
  ...
  if (reviewTriggerModeratorActions.has(moderatorAction.type) && (isActionActive(moderatorAction) || moderatorAction.type === RECEIVED_SENIOR_DOWNVOTES_ALERT)) {
