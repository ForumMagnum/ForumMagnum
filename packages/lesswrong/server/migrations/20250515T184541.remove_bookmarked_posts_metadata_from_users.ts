...
export async function up({ db }: MigrationContext): Promise<void> {
  // This is wrapped in a try/catch because the `bookmarkedPostsMetadata` column was later dropped,
  // so you can't successfully run it in some github actions which bootstrap from accepted_schema.sql,
  // since it doesn't have that column anymore.  (But it's necessary to run if you're on an older FM instance
  // which does still have the column.)
  try {
