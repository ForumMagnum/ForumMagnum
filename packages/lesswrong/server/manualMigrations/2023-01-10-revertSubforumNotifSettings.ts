...
registerMigration({
  ...
  action: async () => {
    ...
    if (Subscriptions.isPostgres()) { // at time of writing, this is true
      ...
      await Subscriptions.rawUpdateMany({_id: {$in: matchingSubscriptionIds}}, {$set: {deleted: true}}, {multi: true})
      ...
      console.log(`Found ${userEditedTagRelIds.length} UserTagRels to revert`)
      ...
      await UserTagRels.rawUpdateMany({_id: {$nin: userEditedTagRelIds}, subforumEmailNotifications: {$ne: defaultValue}}, {$set: {subforumEmailNotifications: defaultValue}}, {multi: true})
    }
