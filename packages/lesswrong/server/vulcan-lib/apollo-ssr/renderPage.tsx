...
const renderRequest = async ({req, user, startTime, res, clientId, userAgent}: RenderRequestParams): Promise<RenderResult> => {
  ...
  return {
    ...
    jssSheets,
    status: renderSideEffects.serverRequestStatus?.status,
    redirectUrl: renderSideEffects.serverRequestStatus?.redirectUrl,
    relevantAbTestGroups: renderSideEffects.abTestGroupsUsed ?? {},
