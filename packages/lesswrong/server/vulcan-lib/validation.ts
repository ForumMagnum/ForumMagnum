...
export const dataToModifier = <T extends DbObject>(data: Partial<DbInsertion<T>>): MongoModifier<T> => ({ 
  $set: pickBy(data, f => f !== null), 
  $unset: mapValues(pickBy(data, f => f === null), () => true),
} as MongoModifier<T>);

export const modifierToData = <T extends DbObject>(modifier: MongoModifier<T>) => ({
  ...modifier.$set,
...
*/
export const validateModifier = <T extends DbObject>(modifier: MongoModifier<T>, document: any, collection: CollectionBase<T>, context: ResolverContext) => {
