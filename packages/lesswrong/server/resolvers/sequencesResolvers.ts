...
/**
 * Deletes an empty chapter, for the sequence's owner or an admin. A sequence's
 * last chapter can't be deleted. Chapters are deleted outright (they have no
 * isDeleted flag), so their description revisions go too: nothing could
 * reach them afterwards.
 */
async function deleteChapter(root: void, { chapterId }: { chapterId: string }, context: ResolverContext): Promise<boolean> {
