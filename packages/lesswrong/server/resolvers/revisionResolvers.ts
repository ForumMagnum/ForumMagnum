...
export function dataToDraftJS(data, type) {
  ...
  switch (type) {
    ...
    case "ckEditorMarkup": {
      ...
      const draftJSContentState = htmlToDraftServer(sanitizeHtml(data), {}, domBuilder)
      return convertToRaw(draftJSContentState)  // On the server have to parse in a JS-DOM implementation to make htmlToDraft work
    }
    case "markdown": {
...
addFieldsDict(Revisions, {
  ...
  plaintextDescription: {
    ...
    resolveAs: {
      type: 'String',
      resolver: ({html}) => {
        const truncatedHtml = truncate(sanitizeHtml(html), PLAINTEXT_HTML_TRUNCATION_LENGTH)
        return htmlToText
