import { getSqlClient, getSqlClientOrThrow } from "@/lib/server/sql/sqlClient";
import Table from "@/lib/server/sql/Table";
import Query from "@/lib/server/sql/Query";
import InsertQuery from "@/lib/server/sql/InsertQuery";
import SelectQuery from "@/lib/server/sql/SelectQuery";
import UpdateQuery from "@/lib/server/sql/UpdateQuery";
import DeleteQuery from "@/lib/server/sql/DeleteQuery";
import CreateIndexQuery from "@/lib/server/sql/CreateIndexQuery";
import DropIndexQuery from "@/lib/server/sql/DropIndexQuery";
import Pipeline from "@/lib/server/sql/Pipeline";
import BulkWriter, { BulkWriterResult } from "@/lib/server/sql/BulkWriter";
import util from "util";
import { getVoteableSchemaFields } from "@/lib/make_voteable";
import { DatabaseIndexSet } from "@/lib//lib/utils/databaseIndexSet";
import TableIndex from "@/lib/server/sql/TableIndex";
...
type ExecuteQueryData<T extends DbObject> = {
    ...
    indexName: string;
    options: MongoFindOptions<T> | MongoUpdateOptions<T> | MongoUpdateOptions<T> | MongoRemoveOptions<T> | MongoEnsureIndexOptions<T> | MongoAggregationOptions | MongoBulkWriteOptions | MongoDropIndexOptions;
};
/**
...
class PgCollection<N extends CollectionNameString = CollectionNameString> implements CollectionBase<N> {
     ...
     */
    async executeQuery(query: Query<ObjectsByCollectionName[N]>, data?: Partial<ExecuteQueryData<ObjectsByCollectionName[N]>>, target: DbTarget = "write"): Promise<any[]> {
        executingQueries++;
        let result: any[];
    ...
    async executeReadQuery(query: Query<ObjectsByCollectionName[N]>, data?: Partial<ExecuteQueryData<ObjectsByCollectionName[N]>>): Promise<any[]> {
        return this.executeQuery(query, data, "read");
    }
    async executeWriteQuery(query: Query<ObjectsByCollectionName[N]>, data?: Partial<ExecuteQueryData<ObjectsByCollectionName[N]>>): Promise<any[]> {
    ...
    private async upsert(selector: string | MongoSelector<ObjectsByCollectionName[N]>, modifier: MongoModifier<ObjectsByCollectionName[N]>, options: MongoUpdateOptions<ObjectsByCollectionName[N]> & {
        upsert: true;
    }) {
    ...
    async rawUpdateOne(selector: string | MongoSelector<ObjectsByCollectionName[N]>, modifier: MongoModifier<ObjectsByCollectionName[N]>, options: MongoUpdateOptions<ObjectsByCollectionName[N]>) {
        if (options?.upsert) {
    ...
    async rawUpdateMany(selector: string | MongoSelector<ObjectsByCollectionName[N]>, modifier: MongoModifier<ObjectsByCollectionName[N]>, options?: MongoUpdateOptions<ObjectsByCollectionName[N]>) {
        const update = new UpdateQuery<ObjectsByCollectionName[N]>(this.getTable(), selector, modifier, options);
        const result = await this.executeWriteQuery(update, { selector, modifier, options });
        return result.length;
    }
    async rawRemove(selector: string | MongoSelector<ObjectsByCollectionName[N]>, options?: MongoRemoveOptions<ObjectsByCollectionName[N]>) {
    ...
    async _ensureIndex(fieldOrSpec: MongoIndexFieldOrKey<ObjectsByCollectionName[N]>, options?: MongoEnsureIndexOptions<ObjectsByCollectionName[N]>) {
        if (!this.table) {
        ...
        if (!options?.concurrently) {
            ...
            // eslint-disable-next-line no-console
            console.warn(`Running CREATE INDEX CONCURRENTLY query without waiting for it to complete, ` +
    ...
    aggregate = (pipeline: MongoAggregationPipeline<ObjectsByCollectionName[N]>, options?: MongoAggregationOptions, comment?: string) => {
        return {
            toArray: async () => {
                try {
    ...
    rawCollection = () => ({
        bulkWrite: async (operations: MongoBulkWriteOperations<ObjectsByCollectionName[N]>, options?: MongoBulkWriteOptions) => {
            executingQueries++;
        ...
        findOneAndUpdate: async (selector: string | MongoSelector<ObjectsByCollectionName[N]>, modifier: MongoModifier<ObjectsByCollectionName[N]>, options: MongoUpdateOptions<ObjectsByCollectionName[N]>) => {
        ...
        updateOne: async (selector: string | MongoSelector<ObjectsByCollectionName[N]>, modifier: MongoModifier<ObjectsByCollectionName[N]>, options: MongoUpdateOptions<ObjectsByCollectionName[N]>) => {
        ...
        updateMany: async (selector: string | MongoSelector<ObjectsByCollectionName[N]>, modifier: MongoModifier<ObjectsByCollectionName[N]>, options: MongoUpdateOptions<ObjectsByCollectionName[N]>) => {
