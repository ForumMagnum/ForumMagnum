...
function extractFragmentDefinitions(fragmentSrc: string): Record<string, string> {
  ...
  for (const match of fragmentDeclarations) {
    ...
    const fragmentText = fragmentSrc.slice(startIndex, traversalIndex);
...
function topologicalSort(fragments: Record<string, string>): string[] {
  ...
  // Return in proper dependency order (dependencies first)
 ...
 * specific arguments.
 */
export class NewSqlFragment {
  private lexer: FragmentLexer;
  private parsedEntries: SqlFragmentEntryMap | undefined;
  private fragmentsByName: Record<string, string>;
  private sqlFragmentsByName: Record<string, NewSqlFragment>;
  ...
  constructor(
     ...
     * the rescue).
     */
    private getSqlFragment: (name: FragmentName) => NewSqlFragment,
    getFragment: (name: FragmentName) => DocumentNode | null,
  ) {
    this.lexer = new FragmentLexer(fragmentSrc);

    // 1. Extract all the fragments from the source text
    // 2. Stash them in a map by fragment name
    this.fragmentsByName = extractFragmentDefinitions(fragmentSrc);
    // 3. Run a topological sort on the fragments to get a dependency order based on what subfragments are referenced
    // Process fragments in dependency order
    const sortedFragmentNames = topologicalSort(this.fragmentsByName);
    
    // Pre-process all fragments in topological order
    // This ensures that when we need a fragment later, it's already been
    // converted to a NewSqlFragment and cached
    for (const fragmentName of sortedFragmentNames) {
      // Only create fragments we don't already have
      if (fragmentName !== this.lexer.getName()) {
        // This will trigger fragment creation and caching via getSqlFragment
    ...
    // 4. Parse them by depth-first traversal
  }
  ...
  private parseEntries(): SqlFragmentEntryMap {
    ...
    while ((line = this.lexer.next())) {
      ...
      if (match?.[1]) {
        ...
        // not a be a totally safe change, so for now we'll just cast.
        const fragmentName = match[1] as FragmentName;
        const fragment = this.getSqlFragment(fragmentName);
        if (!fragment) {
          throw new Error(`Fragment "${fragmentName}" not found`);
        }
        const fragmentEntries = fragment.getParsedEntries();
