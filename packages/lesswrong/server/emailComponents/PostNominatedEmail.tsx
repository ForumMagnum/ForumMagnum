...
export const PostNominatedEmail = async ({documentId, reason, emailContext}: {
  ...
  return (<React.Fragment>
    <p>Your post <a href={postGetAbsolutePageUrl(post, emailContext.resolverContext.forumType)}>{post.title}</a> has received multiple positive votes for the {getReviewNameInSitu()}! On {nominationEndDate.format('MMM Do')}, the nomination vote results will be published, and will be used to help prioritize the Review Phase.</p>
    ...
    <p>You can see a list of nominated posts over at <a href={`${getSiteUrl(emailContext.resolverContext.forumType)}reviewVoting`}>the Review Dashboard page</a>. Posts with at least one review are sorted to the top.</p>
  </React.Fragment>);
}
