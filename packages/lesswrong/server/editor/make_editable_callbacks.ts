...
function addEditableCallbacks<T extends DbObject>({collection, options = {}}: {
    ...
    async function editorSerializationBeforeCreate (doc, { currentUser })
  {
    if (doc[fieldName]?.originalContents) {
      ...
      const newRevision: Omit<DbRevision, "documentId" | "schemaVersion" | "_id" | "voteCount" | "baseScore" | "extendedScore" | "score" | "inactive" | "autosaveTimeoutStart" | "afBaseScore" | "afExtendedScore" | "afVoteCount"> = {
        ...(await buildRevision({
    ...
    async function editorSerializationEdit (docData, { oldDocument: document, newDocument, currentUser })
  {
    if (docData[fieldName]?.originalContents) {
      ...
      if (await revisionIsChange(newDocument, fieldName)) {
        ...
        const newRevision: Omit<DbRevision, '_id' | 'schemaVersion' | "voteCount" | "baseScore" | "extendedScore"| "score" | "inactive" | "autosaveTimeoutStart" | "afBaseScore" | "afExtendedScore" | "afVoteCount"> = {
