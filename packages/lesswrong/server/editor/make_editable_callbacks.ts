...
// export function getEditableCallbacks<N extends CollectionNameString>({collection, options = {}}: {
...
//   // createBefore
//   async function editorSerializationBeforeCreate(doc: DbInsertion<DbType>, {currentUser, context}: CreateCallbackProperties<N>) {
//     const editableField = (doc as AnyBecauseHard)[fieldName] as EditableFieldInsertion | undefined;
//     if (editableField?.originalContents) {
//       if (!currentUser) { throw Error("Can't create document without current user") }
//       const originalContents: DbRevision["originalContents"] = editableField.originalContents
...
//     docData: UpdateData,
//     {oldDocument: document, newDocument, currentUser}: UpdateCallbackProperties<N>,
//   ) {
//     const editableField = (docData as AnyBecauseHard)[fieldName] as EditableFieldUpdate | undefined;
//     if (editableField?.originalContents) {
//       if (!currentUser) { throw Error("Can't create document without current user") }
...
//   // createAfter
//   async function editorSerializationAfterCreate(newDoc: DbType) {
//     // Update revision to point to the document that owns it.
//     const revisionID = (newDoc as AnyBecauseHard)[`${fieldName}_latest`];
//     if (revisionID) {
//       await Revisions.rawUpdateOne(
...
//   async function notifyUsersAboutPingbackMentionsInCreate(newDocument: DbType, {currentUser}: AfterCreateCallbackProperties<N>) {
//     if (currentUser && pingbacks && 'pingbacks' in newDocument) {
...
//   // updateAfter
//   async function notifyUsersAboutPingbackMentionsInUpdate(newDocument: DbType, {oldDocument, currentUser}: UpdateCallbackProperties<N>) {
//     if (currentUser && pingbacks && 'pingbacks' in newDocument) {
...
//    * It's fine to leave it here just in case though
//    */
//   async function reuploadImagesInEdit(doc: DbObject, oldDoc: DbObject) {
//     const latestField = `${fieldName}_latest`;
//     const hasChanged = (oldDoc as AnyBecauseHard)?.[latestField] !== (doc as AnyBecauseHard)?.[latestField];
...
//   // newAsync
//   async function reuploadImagesInNew(doc: DbObject) {
//     await Globals.convertImagesInObject(collectionName, doc._id, fieldName)
//   }
...
//     },
//     updateAfter: {
//       notifyUsersAboutPingbackMentionsInUpdate,
//     },
//     editAsync: {
//       reuploadImagesInEdit,
//     },
//     newAsync: {
//       reuploadImagesInNew,
//       rehostPostMetaImagesInNew,
//     }
//   };
// }
