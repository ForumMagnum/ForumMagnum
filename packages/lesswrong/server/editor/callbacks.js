...
async function PostsNewHTMLSerializeCallback (post) {
  if (post.content) {
    ...
    const newFields = Posts.convertFromHTML(post.htmlBody, post._id, post.slug);
    post = {...post, ...newFields}
  }
  return post
}
...
async function CommentsNewHTMLSerializeCallback (comment) {
  if (comment.content) {
    const newFields = await Posts.convertFromContentAsync(comment.content);
    comment = {...comment, ...newFields}
  } else if (comment.body) {
    const newFields = await Posts.convertFromMarkdownAsync(comment.body)
    comment = {...comment, ...newFields}
  } else if (comment.htmlBody) {
    const newFields = Posts.convertFromHTML(comment.htmlBody);
    comment = {...comment, ...newFields}
  }
  return comment
}
...
async function PostsEditHTMLSerializeCallback (modifier, post) {
  if (modifier.$set && modifier.$set.content) {
    ...
    const newFields = Posts.convertFromHTML(modifier.$set.htmlBody, post._id, post.slug);
    modifier.$set = {...modifier.$set, ...newFields}
  }
  return modifier
}
...
async function CommentsEditHTMLSerializeCallback (modifier, comment) {
  if (modifier.$set && modifier.$set.content) {
