...
HookCollection.prototype = {
  chain: function(hookname: string, func: any) {
    ...
    if (original === identity) {
      ...
      this[hookname] = function(text: string) {
...
export function Converter(this: any, OPTIONS: any = {}): any {
  var pluginHooks = new (HookCollection as any)();
  ...
  var g_urls: any;
  var g_titles: any;
  var g_html_blocks: any;

  // Used to track when we're inside an ordered or unordered list
  // (see _ProcessListItems() for details):
  var g_list_level: number;

  var asciify: any = identity, deasciify: any = identity;
  if (OPTIONS.nonAsciiLetters) {
    ...
    (function() {
      ...
      deasciify = function(text: string) {
  ...
  this.makeHtml = function(text: string) {
    ...
    g_urls = new (SaveHash as any)();
    g_titles = new (SaveHash as any)();
    g_html_blocks = [];
    g_list_level = 0;
  ...
  function _StripLinkDefinitions(text: string) {
    //
  ...
  function _HashHTMLBlocks(text: string) {
    // Hashify HTML blocks:
  ...
  function hashBlock(text: string) {
    text = text.replace(/(^\n+|\n+$)/g, '');
    // Replace the element text with a marker ("~KxK" where x is its key)
    return '\n\n~K' + (g_html_blocks.push(text) - 1) + 'K\n\n';
  }

  function hashMatch(wholeMatch: string, m1: string) {
    return hashBlock(m1);
  }

  var blockGamutHookCallback = function(t: string) { return _RunBlockGamut(t); };

  function _RunBlockGamut(text: string, doNotUnhash = false) {
    //
  ...
  function _RunSpanGamut(text: string) {
    //
  ...
  function _EscapeSpecialCharsWithinTagAttributes(text: string) {
    //
  ...
  function _DoAnchors(text: string) {
  ...
  function writeAnchorTag(wholeMatch: string, m1: string, m2: string, m3: string, m4: string, m5: string, m6: string, m7: string) {
    if (m7 == undefined) m7 = '';
    var whole_match = m1;
  ...
  function _DoImages(text: string) {
  ...
  function attributeEncode(text: string) {
    // unconditionally replace angle brackets here -- what ends up in an attribute (e.g. alt or title)
    // never makes sense to have verbatim HTML in it (and the sanitizer would totally break it)
    return text.replace(/>/g, '&gt;').replace(/</g, '&lt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;');
  }

  function writeImageTag(wholeMatch: string, m1: string, m2: string, m3: string, m4: string, m5: string, m6: string, m7: string) {
    var whole_match = m1;
    var alt_text = m2;
    var link_id = m3.toLowerCase();
    var url = m4;
  ...
  function _DoHeaders(text: string) {
  ...
  function _DoLists(text: string, isInsideParagraphlessListItem = false) {
    //
  ...
  var _listItemMarkers: Record<string,string> = {ol: '\\d+[.]', ul: '[*+-]'};

  function _ProcessListItems(list_str: string, list_type: string, isInsideParagraphlessListItem = false) {
    //
  ...
  function _DoCodeBlocks(text: string) {
    //
  ...
  function _DoCodeSpans(text: string) {
    //
  ...
  function _EncodeCode(text: string) {
    //
  ...
  function _DoItalicsAndBoldStrict(text: string) {
  ...
  function _DoItalicsAndBold_AllowIntrawordWithAsterisk(text: string) {
  ...
  function _DoBlockQuotes(text: string) {
    ...
    text = text.replace(/((^[ \t]*>[ \t]?.+\n(.+\n)*\n*)+)/gm,
      function(wholeMatch, m1) {
        ...
        bq = bq.replace(
        /(\s*<pre>[^\r]+?<\/pre>)/gm,
                              function(wholeMatch: string, m1: string) {
        var pre = m1;
        // attacklab: hack around Konqueror 3.5.4 bug:
  ...
  function _FormParagraphs(text: string, doNotUnhash: boolean) {
    //
    //  Params:
  ...
  function _EncodeAmpsAndAngles(text: string) {
  ...
  function _EncodeBackslashEscapes(text: string) {
    //
    //   Parameter:  String.
  ...
  function handleTrailingParens(wholeMatch: string, lookbehind: boolean, protocol: string, link: string) {
    if (lookbehind)
      return wholeMatch;
  ...
  function _DoAutoLinks(text: string) {
    ...
    var replacer = function(wholematch: string, m1: string) {
  ...
  function _UnescapeSpecialChars(text: string) {
    //
  ...
  function attributeSafeUrl(url: string) {
    url = attributeEncode(url);
    url = escapeCharacters(url, '*_:()[]');
    return url;
  }

  function escapeCharacters(text: string, charsToEscape: string, afterBackslash = false) {
  ...
  function escapeCharacters_callback(wholeMatch: string, m1: string) {
