...
const { createFunction, updateFunction } = getDefaultMutationFunctions('PostViews', {
  ...
  updateFunction: async ({ selector, data }: UpdatePostViewsInput, context, skipValidation?: boolean) => {
    ...
    } = await checkUpdatePermissionsAndReturnProps('PostViews', { selector, context, data, editCheck, schema, skipValidation });
...
const wrappedCreateFunction = wrapMutatorFunction(createFunction, (rawResult, context) => accessFilterSingle(context.currentUser, 'PostViewss', rawResult, context));
const wrappedUpdateFunction = wrapMutatorFunction(updateFunction, (rawResult, context) => accessFilterSingle(context.currentUser, 'PostViewss', rawResult, context));
