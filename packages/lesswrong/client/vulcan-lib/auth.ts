...
import { onStartup, deferWithoutDelay, meteorLocalStorageWrapper } from '../../lib/executionEnvironment';
...
if (meteorLocalStorageWrapper) {
  const originalSetItem = meteorLocalStorageWrapper.setItem;
  meteorLocalStorageWrapper.setItem = function setItem(key: string, value: string) {
    if (key === 'Meteor.loginToken') {
      deferWithoutDelay(initToken);
    }
    originalSetItem.call(meteorLocalStorageWrapper, key, value);
  };
  const originalRemoveItem = meteorLocalStorageWrapper.removeItem;
  meteorLocalStorageWrapper.removeItem = function removeItem(key: string) {
    if (key === 'Meteor.loginToken') {
      deferWithoutDelay(initToken);
    }
    originalRemoveItem.call(meteorLocalStorageWrapper, key);
  };
}
