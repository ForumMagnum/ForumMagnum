...
import { createHttpLink, createErrorLink, createHeaderLink } from '../lib/apollo/links';
...
export const createApolloClient = (baseUrl = '/'): ApolloClient<NormalizedCacheObject> => {
  ...
  return new ApolloClient({
    link: ApolloLink.from([createHeaderLink, createErrorLink(), createHttpLink(baseUrl)]),
    cache,
  });
};
