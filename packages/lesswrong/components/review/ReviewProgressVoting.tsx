...
export const TARGET_REVIEW_VOTES_NUM = 6
...
export const ReviewProgressVoting = ({classes, reviewYear}: {
  ...
  const { results: reviewsResults, totalCount } = useMulti({
    ...
    enableTotal: true,
    skip: !currentUser,
    limit: TARGET_REVIEW_VOTES_NUM
  });

  if (!reviewsResults) return null

  const totalVotes = totalCount || 0
  const uncheckedBoxes = TARGET_REVIEW_VOTES_NUM - Math.min(totalVotes, TARGET_REVIEW_VOTES_NUM)

  return <div><LWTooltip title={<div>
    <p><em>{totalVotes ? `You've voted on ${totalVotes} posts in the ${reviewYear} Review${totalVotes >= TARGET_REVIEW_VOTES_NUM ? "!" : "."}` : "You haven't voted on any posts yet."}</em></p>
    {totalVotes < TARGET_REVIEW_VOTES_NUM && <>
    <div>It'd be helpful if you voted on at least {TARGET_REVIEW_VOTES_NUM} posts. More votes from established users improves the signal/noise of the review vote. (If there weren't {TARGET_REVIEW_VOTES_NUM} posts you found longterm valuable, it's fine to vote "0").</div></>}</div>} placement="top">
