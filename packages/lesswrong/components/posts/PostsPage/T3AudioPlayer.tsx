...
const styles = defineStyles('T3AudioPlayer', (theme: ThemeType) => ({
  embeddedPlayer: {
    marginBottom: "30px"
  },
  hideEmbeddedPlayer: {
...
const type3PlayerIsPlaying = (root: HTMLElement | null) => {
  return !!root?.querySelector('.t3a-player--playing');
}
...
export const T3AudioPlayer = ({showEmbeddedPlayer, documentId, collectionName, onPlaybackChange}: {
  ...
  useEffect(() => {
    if (!type3scriptLoaded || !divRef.current || typeof MutationObserver === "undefined") {
      return;
    }

    const root = divRef.current;
    const updatePlaybackState = () => {
      const nextIsPlaying = type3PlayerIsPlaying(root);
      if (lastPlaybackState.current === nextIsPlaying) {
        return;
      }

      const previousPlaybackState = lastPlaybackState.current;
      lastPlaybackState.current = nextIsPlaying;
      setIsPlaying(nextIsPlaying);

      if (nextIsPlaying || previousPlaybackState === true) {
    ...
    const observer = new MutationObserver(updatePlaybackState);
  ...
  }, [type3scriptLoaded, onPlaybackChange]);
  ...
  >
    <div className={classNames(classes.embeddedPlayer, { [classes.hideEmbeddedPlayer]: !showEmbeddedPlayer && !isPlaying })}>
      {type3scriptLoaded && (
        /* @ts-ignore */
