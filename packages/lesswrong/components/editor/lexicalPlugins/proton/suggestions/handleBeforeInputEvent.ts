...
import {
  ...
  $createTabNode,
  $createParagraphNode,
  type LexicalEditor, type ElementNode, type RangeSelection, type LexicalNode
} from 'lexical'
...
export function $handleBeforeInputEvent(
  ...
  logger: Logger,
  createNotification?: (options: WithMessagesMessage) => void,
): boolean {
  const inputType = event.inputType

  logger('handleBeforeInput', inputType)
  ...
  if (!$isRangeSelection(selection)) {
    logger('Current selection is not a range selection', selection)
    return true
  }
  ...
  if ($isAnyPartOfSelectionInCodeNode(selection)) {
    logger('Aborting beforeinput because selection is inside a code-block')
    createNotification?.({
...
function $handleDeleteInput(
  ...
  if (!currentBlock) {
    logger('Could not find block parent')
    return true
  }
  ...
  if (isAtStartOfBlock) {
    logger('Selection is collapsed at start of block')
    ...
    if ($isDecoratorNode(previousSibling)) {
      logger('Previous sibling is decorator node')
    ...
    if (previousBlock) {
      if (previousBlock.isEmpty()) {
        logger('Previous non-inline leaf element is empty')
      ...
      if (splitSuggestion) {
        logger('Previous non-inline leaf element has split suggestion')
  ...
  if (!suggestionNodes.length) {
    logger('No suggestion nodes were created')
    return true
  }
  ...
  if (suggestionNodes.length === 1) {
    ...
    if ($isSuggestionNode(prevSibling) && prevSibling.getSuggestionTypeOrThrow() === 'delete') {
      ...
      logger('Merged delete suggestion with prev delete sibling')
      ...
      logger('Merged delete suggestion with next delete sibling')
    } else {
      onSuggestionCreation(suggestionID)
    }
  } else {
...
function $handleInsertInput(
  ...
  if (!selection.isCollapsed() && data !== null && dataTransfer === null) {
    logger('Wrapping non-collapsed selection in a delete suggestion')
    ...
    if (isInsideExistingSelection && existingParentSuggestion?.getSuggestionTypeOrThrow() === 'insert') {
      logger('Removing the wrapped suggestion as it is inside an existing one')
  ...
  if (!$isRangeSelection(latestSelection)) {
    logger('Latest selection is not range selection')
    return true
  }
  ...
  if (
    ...
    logger('Applying targetRange to current selection')
  ...
  if (data === null && dataTransfer !== null) {
    ...
    logger('Inserting data transfer', types)
...
function $handleInsertParagraph(
  ...
  logger('Created new paragraph by splitting existing one and added split suggestion to the previous')
...
function $handleInsertParagraphOnEmptyListItem(
  ...
  logger('Inserting paragraph on empty list item')
  ...
  if (!didInsert) {
    logger('Did not insert paragraph')
...
function $handleInsertTextData(
  ...
  logger('Inserting text data: ', data)
  ...
  if (isExistingSuggestionEmpty) {
    logger('Selection is inside empty suggestion')
    ...
    if ($isNonInlineLeafElement(existingParentSuggestion.getParentOrThrow())) {
      logger('Inserting suggestion node after existing suggestion')
      existingParentSuggestion.insertAfter(suggestionNode)
    } else {
      logger("Cannot insert suggestion in existing suggestion's parent")
      ...
      if (sibling && $isNonInlineLeafElement(sibling)) {
        logger('Inserting new suggestion into element sibling')
        sibling.append(suggestionNode)
      } else {
        logger('Creating new paragraph and inserting suggestion')
  ...
  if (shouldSplitExistingSuggestionBeforeInserting) {
    logger('Will split existing non-insert suggestion and insert new insert suggestion')
    ...
    if (nodeToInsertBefore) {
      ...
      logger('Could not find node to insert before')
  ...
  if (existingParentSuggestion && canInsertTextDataDirectly) {
    logger('Will just insert text as already inside insert suggestion')
    if (isFocusNodeText) {
  ...
  logger('Created and inserted new insert suggestion')
  ...
  if (suggestionSibling) {
    ...
    if (shouldMergeWithSibling) {
      ...
      logger('Merged with existing insert suggestion sibling')
      if (isNextSiblingSuggestion) {
        logger('Updating cursor position after merging')
...
function $handleReplacementTextInsideInsertSuggestion(
  ...
  logger('Will insert replacement text', newText)
  ...
  if (!$isTextNode(node)) {
    logger('Current anchor node is not text node', node)
    return true
  }
