...
export function $setTextNodeFormatAsSuggestion(
  ...
  if ($isSuggestionNode(parent)) {
    logger('Directly updating format as node is in suggestion')
    node.setFormat(format)

    const isFormatChangeSuggestion = parent.getSuggestionTypeOrThrow() === 'property-change'
    if (isFormatChangeSuggestion) {
      logger('Existing suggestion is format change suggestion')
      ...
      if (latestFormat === formatSuggestionInitialFormat) {
        logger('Unwrapping existing suggestion since format was reset to original')
  ...
  logger('Updating format and wrapping node in suggestion node')
...
export function $formatTextAsSuggestion(
  ...
  logger('Formatting text as suggestion', format)

  const selection = $getSelection()
  if (!$isRangeSelection(selection)) {
    logger('Expected range selection')
    return true
  }

  if (selection.isCollapsed()) {
    logger('Directly setting selection format since it is collapsed')
  ...
  if (selectedTextNodesLength === 0) {
    logger('Directly setting selection format since no text nodes selected')
  ...
  if (firstNode == null) {
    logger('No usable first node found')
    return true
  }
  ...
  if (isSingleNodeSelected) {
    logger('Only single node selected')

    const noTextIsSelected = startOffset === endOffset
    if (noTextIsSelected) {
      logger('No text selected, skipping')
      return true
    }
    ...
    if ($isTokenOrSegmented(firstNode) || isEntireTextSelected) {
      logger('Node is token or is fully selected')
      ...
      logger('Splitting partially selected node and setting format to the selected')
    ...
    logger('Multiple nodes are selected')

    if (startOffset !== 0 && !$isTokenOrSegmented(firstNode)) {
      logger('First node is partially selected and is not a token, splitting')
