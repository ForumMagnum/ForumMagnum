...
import { type SuggestionProperties, type SuggestionType, ProtonNodeTypes } from './Types'
...
export class ProtonNode extends ElementNode {
  ...
  getSuggestionChangedProperties<T extends Record<string, AnyBecauseHard>>(): T | undefined {
    const props = this.__properties
    if (props.nodeType !== 'suggestion') {
      throw new Error('Node is not suggestion node')
    }
