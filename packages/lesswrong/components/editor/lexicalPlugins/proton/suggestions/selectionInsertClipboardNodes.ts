...
export function $selectionInsertClipboardNodes(
  ...
  logger('Inserting nodes at current selection', nodes)
  ...
  logger('Is selection backward?', isInitialSelectionBackward, selection.anchor.offset, selection.focus.offset)
  ...
  if (isInitialSelectionNotCollapsed) {
    ...
    logger('Wrapped non-collapsed selection as delete suggestion', suggestionID)
  }

  const latestSelection = $getSelection()
  if (!$isRangeSelection(latestSelection) || !latestSelection.isCollapsed()) {
    logger('Latest selection is not range selection or is not collapsed')
    return true
  }
  ...
  if (allNodesAreInline) {
    ...
    logger('Appended all nodes as one suggestion node as all nodes were inline')
    ...
    if ($isSuggestionNode(sibling) && sibling.getSuggestionTypeOrThrow() === 'delete') {
      logger('Using same suggestion ID as sibling to create replace suggestion')
  ...
  logger('Anchor before inserting:', anchorBeforeInserting)

  logger('Inserting nodes into latest selection')
  ...
  logger('Anchor after inserting:', anchorAfterInserting)
  ...
  if (isAtStartOfBlock || !doesBeforeAnchorExist) {
    ...
    logger('Anchor before insertion doesnt exist, using first inserted node')
    const firstNode = nodes[0]
    selectionToMark.anchor.set(firstNode.getKey(), 0, $isTextNode(firstNode) ? 'text' : 'element')
  } else {
    logger('Anchor before insertion still exists, using that.')
  ...
  logger('Selection to wrap:', selectionToMark)
  ...
  if (!suggestionNodes.length) {
    logger('Selection was not wrapped')
    return true
  }
  ...
  if (suggestionIDtoUseForReplace) {
    logger('Creating replace suggestion by using same suggestion ID as delete suggestion sibling')
    for (const suggestion of suggestionNodes) {
      suggestion.setSuggestionId(suggestionIDtoUseForReplace)
    }
  } else {
    logger('Wrapped selection in suggestion', suggestionID)
    onSuggestionCreation(suggestionID)
  }
