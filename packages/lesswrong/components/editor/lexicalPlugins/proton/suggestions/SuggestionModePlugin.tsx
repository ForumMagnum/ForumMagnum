...
import { SuggestionTypesThatCanBeEmpty, type SuggestionID, type EditorRequiresClientMethods } from './Types'
import { BEFOREINPUT_EVENT_COMMAND, COMPOSITION_START_EVENT_COMMAND, INSERT_FILE_COMMAND } from '../utils/commands'
...
export function SuggestionModePlugin({
  ...
  const [suggestionModeLogger] = useState(() => loggerConstructor('docs-suggestions-mode'))
    ...
    return mergeRegister(
      ...
      editor.registerMutationListener(ProtonNode, (mutations) => {
        ...
        for (const id of idsToCreateCommentsFor) {
          ...
          suggestionModeLogger(`Creating new thread for suggestion ${id}`)
          ...
          void controller
            ...
            .then(() => {
              ...
              suggestionModeLogger(`Removed id ${id} from set ${[...createdSuggestionIDs]}`)
            })
      ...
      void controller
        ...
        .then((threads) => {
          for (const added of reAddedNodes) {
            ...
            suggestionModeLogger(`Reopening thread ${thread.id} for suggestion ${suggestionID} after undo/redo`)
            void controller.reopenSuggestion(thread.id).catch(captureException)
          }
          for (const removed of removedNodes) {
            ...
            suggestionModeLogger(`Rejecting thread ${thread.id} for suggestion ${suggestionID} after undo/redo`)
            void controller.rejectSuggestion(thread.id).catch(captureException)
          }
        })
        .catch(captureException)
    }
    ...
    return mergeRegister(
      ...
      editor.registerNodeTransform(ProtonNode, function cleanupEmptySuggestionNodes(node) {
        ...
        if (node.getChildrenSize() === 0) {
          ...
          suggestionModeLogger('Removing empty suggestion node', id, type)
          node.remove()
        }
      }),
    )
  ...
  useEffect(() => {
    ...
    const addCreatedIDtoSet = (id: string) => {
      ...
      suggestionModeLogger('Created suggestion node with ID: ', id)
    }
    ...
    return mergeRegister(
      ...
      editor.registerCommand(
        ...
        (event) => {
          return $handleBeforeInputEvent(editor, event, addCreatedIDtoSet, suggestionModeLogger, flash)
        },
        COMMAND_PRIORITY_CRITICAL,
      ),
      editor.registerCommand(
      ...
      editor.registerCommand(
        ...
        function disableSuggestionModeIfComposing() {
          suggestionModeLogger('Editor is composing, disabling suggestion mode')
          ...
          //   translatedMessage: "The language you're using isn't currently supported in suggestion mode. Please switch to edit mode or change the language.",
          // })
          return true
        },
        COMMAND_PRIORITY_CRITICAL,
  ...
  }, [controller, editor, isSuggestionMode, onUserModeChange, flash, suggestionModeLogger])
