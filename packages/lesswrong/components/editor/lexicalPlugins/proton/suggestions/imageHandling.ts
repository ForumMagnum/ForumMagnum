...
export function $insertImageNodeAsSuggestion(
  ...
  logger('Insert image node as suggestion')
  const selection = $getSelection()
  if (!selection) {
    logger('No selection available')
    return true
  }
...
export function $handleImageSizeChangeAsSuggestion(
  ...
  logger('Handling image size change', payload)
  const node = $getNodeByKey(nodeKey)
  if (!$isImageNode(node)) {
    logger('Node is not image node')
    return true
  }
  const initialWidth = node.getWidth()
  const initialHeight = node.getHeight()
  logger('Setting new width and height')
  ...
  logger('Wrapping node with new suggestion', initialWidth, initialHeight)
  const suggestionID = randomId()
...
export function $handleImageDragAndDropAsSuggestion(
  ...
  if (!draggedImageNode) {
    logger('No dragged image node')
    return false
  }
  const data = getDragImageData(event)
  if (!data) {
    logger('Could not get image data from event')
    return true
  }
  event.preventDefault()
  if (!$canDropImage(event)) {
    logger('Cannot drop image')
    return true
  }
  const suggestionID = randomId()
  const range = getDragSelection(event)
  logger('Wrapping existing node with "delete" type')
  ...
  logger('Created and inserted "insert" type suggestion')
