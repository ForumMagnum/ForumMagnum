...
export function $wrapSelectionInSuggestionNode(
  ...
  logger?.('Wrapping selection in suggestion node', info)
  ...
  for (let i = 0; i < nodesLength; i++) {
    ...
    if ($isElementNode(lastCreatedMarkNode) && lastCreatedMarkNode.isParentOf(node)) {
      logger?.('Last created suggestion node is parent of current node')
      continue
    }
    ...
    if ($isTextNode(node)) {
      ...
      logger?.('Splitting text node', loggerInfo)

      if (startTextOffset === 0 && endTextOffset === 0) {
        logger?.('Not splitting text node because start and end offset are both 0')
        continue
      }
      ...
      if (isAtEndOfTextNode) {
        ...
        if (type === 'delete' && isLastDescendant) {
          logger?.('Adding join node because start offset is at end of text node which is also the last descendant')
        ...
        logger?.('Not splitting text not because at end of text node')
        continue
      }
      ...
      logger?.('Ignoring existing suggestion node')
      continue
    } else if ($isElementNode(node) && node.isInline()) {
      logger?.('Node is inline element node')

      const isFullyWithinSelection = !node.isParentOf(anchor.getNode()) && !node.isParentOf(focus.getNode())
      if (isFullyWithinSelection) {
        logger?.('Node is fully within selection')
        targetNode = node
      }
    } else if ($isImageNode(node)) {
      logger?.('Node is image node')
      targetNode = node
    } else if ($isTableNode(node)) {
      logger?.('Node is table node')
    ...
    if (targetNode !== null) {
      if (targetNode && targetNode.is(currentNodeParent)) {
        logger?.('Current node is a child of the target node to be wrapped')
        continue
      }

      const parentNode = targetNode.getParent()
      if (parentNode == null || !parentNode.is(currentNodeParent)) {
        logger?.("Parent node is not the current node's parent node")
      ...
      if (lastCreatedMarkNode === null) {
        logger?.('Creating new suggestion node')
        lastCreatedMarkNode = $createSuggestionNode(id, type, changedProperties)
        targetNode.insertBefore(lastCreatedMarkNode)
        createdMarkNodes.push(lastCreatedMarkNode)
      }

      logger?.('Appending target node to last created suggestion node', lastCreatedMarkNode)
      lastCreatedMarkNode.append(targetNode)
    } else {
      logger?.('Clearing state because no target node found')
