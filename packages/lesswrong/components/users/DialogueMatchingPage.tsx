...
import { RecommendedComment, TagWithCommentCount } from '../dialogues/DialogueRecommendationRow';
...
const UserPostsYouveRead = ({ classes, targetUserId, hideAtSm, limit = 20}: { classes: ClassesType<typeof styles>, targetUserId: string, hideAtSm: boolean, limit?: number }) => {
  ...
  const { loading, error, data } = useQuery('UsersReadPostsOfTargetUser', {
    variables: { userId: currentUser!._id, targetUserId: targetUserId, limit : limit },
    skip: !currentUser
  });

  const readPosts = data?.UsersReadPostsOfTargetUser
...
const UserTopTags = ({ classes, targetUserId }: { classes: ClassesType<typeof styles>, targetUserId: string }) => {
  ...
  const { loading, error, data } = useQuery('UserTopTags', {
...
const UserRecommendedContent = ({ classes, targetUserId }: UserRecommendedContentProps ) => {
  ...
  const { loading: tagLoading, error: tagError, data: tagData } = useQuery('UserTopTags', {
  variables: { userId: targetUserId },
  skip: !currentUser
});

const { loading: postsLoading, error: postsError, data: postsData } = useQuery('UsersReadPostsOfTargetUser', {
  variables: { userId: currentUser!._id, targetUserId: targetUserId, limit : 4 },
  skip: !currentUser
});

const { loading: commentsLoading, error: commentsError, data: commentsData } = useQuery('UsersRecommendedCommentsOfTargetUser', {
  variables: { userId: currentUser!._id, targetUserId: targetUserId, limit : 3 },
  skip: !currentUser
});

const topTags = tagData?.UserTopTags;
const readPosts = postsData?.UsersReadPostsOfTargetUser
...
const NextStepsDialog = ({ onClose, userId, targetUserId, targetUserDisplayName, matchPreference, reciprocalMatchPreference, dialogueCheckId, classes }: NextStepsDialogProps) => {
  ...
  const { loading, error, data } = useQuery('GetTwoUserTopicRecommendations', {
    variables: { userId, targetUserId, limit:4 },
  });


  const topicRecommendations = data?.GetTwoUserTopicRecommendations; // Note CommentsList is too permissive here, but making my own type seemed too hard
...
export const DialogueMatchingPage = ({classes}: {
  ...
  const { loading, error, data } = useQuery('GetUserDialogueUsefulData', {skip: !currentUser || !dialogueMatchmakingEnabled.get()});
  ...
  const userDialogueUsefulData = data?.GetUserDialogueUsefulData;
