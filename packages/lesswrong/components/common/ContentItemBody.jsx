...
const lesswrongRegex = /(http:\/\/lesswrong\.com\/)((?:[a-z][a-z0-9_]*))/i
const processingInstructions = [
    {
     ...
     shouldProcessNode: (node) => {
        const shouldProcess = node.type === "tag" && node.name === "a"
          && node.attribs && node.attribs.href &&
          (lesswrongRegex.test(node.attribs.href) || node.attribs.href[0] === "/")
        if (shouldProcess) console.log("processing node: ", node)
        return shouldProcess
      },
      //eslint-disable-next-line react/display-name
      processNode: function (node, children, index) {
        // Extracts relative url from href string
        if (node.type === "tag" && node.name === "a" && node.attribs.href[0] === "/") {
          return <Link key={index} to={node.attribs.href}>{children}</Link>
        } else {
          const relativeURL = node.attribs.href.split('/').splice(3).join('/')
          return <Link key={index} to={relativeURL}> {children} </Link>
        }
      }
    },
    {
...
class ContentItemBody extends Component {
  ...
  render() {
    ...
    const htmlInput = this.props.dangerouslySetInnerHTML['__html']
    return htmlToReactParser.parseWithInstructions(htmlInput, isValidNode, processingInstructions)
  }
