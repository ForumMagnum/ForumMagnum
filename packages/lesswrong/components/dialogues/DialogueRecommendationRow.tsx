...
type ValidatedTopicRecommendation = QueryGetTwoUserTopicRecommendationsResult_GetTwoUserTopicRecommendations & {
  theirVote: 'agree' | 'disagree';
};
...
const DialogueRecommendationRow = ({ targetUser, checkId, userIsChecked, userIsMatched, classes, showSuggestedTopics, onHide }: DialogueRecommendationRowProps) => {
  ...
  const { loading, error, data: topicData } = useQuery('GetTwoUserTopicRecommendations', {
  ...
  const recommendedComments = commentsData?.UsersRecommendedCommentsOfTargetUser

  const preTopicRecommendations = topicData?.GetTwoUserTopicRecommendations; 
  const topicRecommendations = preTopicRecommendations?.filter((topic): topic is ValidatedTopicRecommendation => ['agree', 'disagree'].includes(topic.theirVote ?? '') ); // todo: might want better type checking here in future for values of theirVote
