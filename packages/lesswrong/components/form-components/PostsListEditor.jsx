...
const SortablePostItem = withStyles(styles, {name: "SortablePostItem"})(
  // React sortable has constructors that don't work like normal constructors
  //eslint-disable-next-line babel/new-cap
  SortableElement(({postId, currentUser, removeItem, classes}) =>
...
// React sortable has constructors that don't work like normal constructors
//eslint-disable-next-line babel/new-cap
const SortablePostList = SortableContainer(({items, currentUser, removeItem}) => {
  return (
    <div>
      {items.map((postId, index) => (
        <SortablePostItem key={`item-${index}`} removeItem={removeItem} index={index} postId={postId} currentUser={currentUser}/>
      ))}
    </div>
  );
});
