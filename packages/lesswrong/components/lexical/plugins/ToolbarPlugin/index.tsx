...
const LIST_STYLE_OPTIONS: [string, string][] = [
  ['decimal', '1, 2, 3'],
  ['lower-alpha', 'a, b, c'],
  ['upper-alpha', 'A, B, C'],
...
export default function ToolbarPlugin({
    ...
    if ($isRangeSelection(selection)) {
      ...
      if (elementDOM !== null) {
        ...
        if ($isListNode(element)) {
          ...
          if (type === 'number') {
            const activeList = (parentList ?? element) as ListNode;
            const style = activeList.getStyle();
            const match = style.match(/list-style-type:\s*([^;]+)/);
            updateToolbarState('listStyleType', match ? match[1].trim() : 'decimal');
          }
        } else {
  ...
  const onListStyleTypeSelect = useCallback(
    (value: string) => {
      activeEditor.update(() => {
        $addUpdateTag(SKIP_SELECTION_FOCUS_TAG);
        const selection = $getSelection();
        ...
        if (parentList && parentList.getListType() === 'number') {
          parentList.setStyle(value === 'decimal' ? '' : `list-style-type: ${value}`);
        }
      });
    },
    [activeEditor],
          ...
          {toolbarState.blockType === 'number' && (
            <>
              <DropDown
                disabled={!isEditable}
                buttonClassName={classes.codeLanguage}
                buttonLabel={LIST_STYLE_OPTIONS.find(([v]) => v === toolbarState.listStyleType)?.[1] ?? '1, 2, 3'}
                buttonAriaLabel="Select list numbering style">
                {LIST_STYLE_OPTIONS.map(([value, name]) => (
                  <DropDownItem
                    key={value}
                    active={toolbarState.listStyleType === value}
                    onClick={() => onListStyleTypeSelect(value)}>
                    <DropDownItemText>{name}</DropDownItemText>
                  </DropDownItem>
                ))}
              </DropDown>
