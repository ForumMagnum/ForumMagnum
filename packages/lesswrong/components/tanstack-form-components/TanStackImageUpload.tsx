...
const styles = defineStyles('TanStackImageUpload', (theme: ThemeType) => ({
  root: {
    paddingTop: 4,
    marginLeft: 8,
    display: "flex",
...
/**
 * FormProfileImage: Render a user's profile image given a user _id plus a
 * new `profileImageId`.
 */
...
/**
 * TriggerButton: wraps MUI Button to open the upload dialog.
 */
...
/**
 * RemoveButton: wraps MUI Button to clear the current image.
 */
const RemoveButton: FC<{
  ...
  const mainClass =
    isFriendlyUI && imageType === "profileImageId"
...
/**
 * TanStackImageUpload: A field component for uploading and previewing images.
 */
export function TanStackImageUpload({
  field,
  document,
  label,
  croppingAspectRatio,
  ...
  return (
    <div className={classes.root}>
      ...
      >
          ...
          <Button
            ...
            onClick={() =>
              openDialog({
                ...
                contents: ({ onClose }) => (
                  <Components.ImageUploadDefaultsDialog
                    onClose={onClose}
        ...
        {userHasDefaultProfilePhotos(currentUser) &&
