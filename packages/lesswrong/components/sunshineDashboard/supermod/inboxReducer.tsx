...
export type InboxState = {
  ...
  // New-content user IDs that have migrated to the Pangram tab, populated client-side
  // as each user's content loads. See userContentQualifiesForPangram.
...
export function getFilteredGroups(
  ...
  const pangramUserIdSet = new Set(pangramQualifyingUserIds);
  ...
  // Pangram is a subtab of New Content showing only qualifying new-content users.
  if (activeTab === 'pangram') {
    const pangramUsers = (groupedUsers.newContent ?? []).filter(user => pangramUserIdSet.has(user._id));
    return pangramUsers.length > 0 ? [['newContent', pangramUsers]] : [];
  }

  // Qualifying users live in the Pangram tab, so exclude them from New Content elsewhere.
  const groupsWithoutPangramNewContent = orderedGroups
    .map(([group, users]): GroupEntry => (
      group === 'newContent' ? [group, users.filter(user => !pangramUserIdSet.has(user._id))] : [group, users]
    ))
...
export function getVisibleTabsInOrder(
  ...
  const qualifyingInNewContent = (groupedUsers.newContent ?? []).filter(user => pangramUserIdSet.has(user._id)).length;
  const tabs: TabInfo[] = [{ group: 'curation', count: totalCurationNotices }];
  
  // Always show all tabs, even if empty. Pangram sits right after New Content,
  // and its users are excluded from the New Content (and All) counts.
  for (const group of tabsInOrder) {
    ...
    if (group === 'newContent') {
      tabs.push({ group: 'pangram', count: qualifyingInNewContent });
    }
  }
...
export function inboxStateReducer(state: InboxState, action: InboxAction): InboxState {
  switch (action.type) {
    ...
    case 'CHANGE_TAB': {
      ...
      const filteredGroups = getFilteredGroups(groupedUsers, action.tab, state.pangramQualifyingUserIds);
    ...
    case 'NEXT_USER': {
      ...
      const groupedUsers = groupBy(state.users, user => getUserReviewGroup(user));
      const filteredGroups = getFilteredGroups(groupedUsers, state.activeTab, state.pangramQualifyingUserIds);
      const orderedUsers = filteredGroups.flatMap(([_, users]) => users);
    ...
    case 'NEXT_TAB': {
      ...
      const visibleTabs = getVisibleTabsInOrder(groupedUsers, state.users.length, state.posts.length, state.classifiedPosts.length, curationNoticeCount, state.pangramQualifyingUserIds);
      ...
      // Switching to a user tab
      const filteredGroups = getFilteredGroups(groupedUsers, nextTab, state.pangramQualifyingUserIds);
      const orderedUsers = filteredGroups.flatMap(([_, users]) => users);
    ...
    case 'PREV_TAB': {
      ...
      // Switching to a user tab
      const filteredGroups = getFilteredGroups(groupedUsers, prevTab, state.pangramQualifyingUserIds);
      const orderedUsers = filteredGroups.flatMap(([_, users]) => users);
    ...
    case 'REMOVE_USER': {
      ...
      if (orderedUsers.length > 0) {
        ...
        const oldFilteredGroups = getFilteredGroups(oldGroupedUsers, state.activeTab, state.pangramQualifyingUserIds);
      ...
      const nextFilteredGroups = getFilteredGroups(groupedUsers, nextTab, state.pangramQualifyingUserIds);
