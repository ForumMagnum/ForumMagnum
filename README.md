...
7. [Lexical](https://lexical.dev/) is the default rich text editor for posts, comments, and some other form fields; it's shown to users as "LessWrong Docs". [CkEditor5](https://ckeditor.com/) was the previous default and is still supported for documents that were written in it, and markdown and raw HTML are also available as editor types. [Draft](https://draftjs.org/) is no longer a supported editor; documents that were last saved in DraftJS are converted to Lexical when they're opened.
