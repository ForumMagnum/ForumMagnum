...
# Load the front page
if ! curl --silent --show-error -o tmp/localFrontPage $local_frontpage_url; then
...
for scripturl in $(cat tmp/scriptFilenames); do
  ...
  curl --silent --show-error -o "tmp/bundleSizeDownloads/${scripturl_noslashes_nohash}" "http://localhost:3000$scripturl"
done
