...
export async function main() {
  ...
  const componentFiles = filter(allSourceFiles, f => {
    ...
    return relativePath.toLowerCase().endsWith(".tsx")
      && relativePath.startsWith("packages/lesswrong/components/")
  });
...
function getPathForImport(sourceFile: SourceFile) {
  ...
  return relativePath.replace("packages/lesswrong/components/", "@/components/").replace(/\.tsx$/, "");
}

function importPathToAbsolute(importPath: string) {
  return importPath.replace("@/components", cwd+"/packages/lesswrong/components");
}
